...
def _stringify_uuids(obj):
    ...
    if isinstance(obj, list):   return [_stringify_uuids(item) for item in obj]
    if isinstance(obj, tuple):  return tuple(_stringify_uuids(item) for item in obj)
    if isinstance(obj, dict):   return {key: _stringify_uuids(value) for key, value in obj.items()}
    return obj
