...
async def select_ticket_by_id(session: AsyncSession, ticket_id: UUID) -> Tickets | None:
    """
    Retorna el ticket sin joins para inspeccionar campos bA!sicos
    (requester_id, manager_id, group_id, status_type_id, etc.).
    """
    statement = (
        select(Tickets)
        .where(
...
async def select_total_tickets_by_group_id(
    session: AsyncSession,
    group_id: UUID,
) -> UserAccounts | None:
    statement = (
        select(Tickets)
        .select_from(Tickets)
        .where(and_(Tickets.group_id == group_id, Tickets.active == True))
    )
    result = await session.execute(statement)
    ...
    data = [
        {
            ...
            'resolution': object_model.resolution,
...
async def select_user_emails(session: AsyncSession, user_account_id: UUID) -> list:
    emails = []

    requester_email = await select_user_email_by_user_account_id(session, user_account_id)
    if requester_email:
        emails.append(requester_email)

    support_emails = await select_all_support_user_emails(session)
    if len(support_emails) > 0:
        for support_email in support_emails:
            if support_email not in emails:
...
async def select_all_support_user_emails(session: AsyncSession) -> list[str]:
    statement = (
        select(UserProfiles.email)
        .select_from(UserAccounts)
        .join(UserGroups, UserGroups.id == UserAccounts.user_group_id)
        ...
        .where(
            ...
            UserGroups.description == 'Soporte',
        )
        .correlate(None)
    )
    result = await session.execute(statement)
...
async def select_all_support_users(session: AsyncSession):
    statement = (
        select(
    ...
    #payload: list[dict] = [
    #    {'id': str(user_id), 'username': username, 'full_name': full_name, 'email': email}
    #    for user_id, username, full_name, email in result.all()
    #]
    #return payload
